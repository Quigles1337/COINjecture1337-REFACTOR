#!/usr/bin/env python3
"""
Migrate CIDs in JSON Block Data
Converts CIDs stored in block_bytes JSON from regular base58 to base58btc
"""
...
class JSONCIDMigrator:
    ...
    def save_migration_log(self, results: Dict[str, int]) -> str:
        ...
        log_file = f"json_cid_migration_log_{int(time.time())}.json"
    ...
    def run_migration(self) -> bool:
        ...
        print("🎯 COINjecture JSON CID Migration")
        print("=" * 40)
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python migrate_json_cids.py <database_path>")
        print("Example: python migrate_json_cids.py /opt/coinjecture/data/blockchain.db")
        sys.exit(1)
    
    db_path = sys.argv[1]
    
    migrator = JSONCIDMigrator(db_path)
    success = migrator.run_migration()
    
    if success:
        print("\n✅ JSON CID migration completed successfully!")
    else:
        print("\n❌ JSON CID migration completed with errors.")
