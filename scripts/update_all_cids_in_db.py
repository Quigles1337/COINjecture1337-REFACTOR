#!/usr/bin/env python3
"""
Update All CIDs in Database Script

Updates all existing blocks in the database to use valid base58btc CIDs
by connecting to the server database and updating the block_bytes JSON.
"""
...
class DatabaseCIDUpdater:
    ...
    def update_block_cid_via_api(self, height, new_cid):
        """Update block CID by re-ingesting with new CID"""
        try:
            ...
            self.log(f"⚠️  Error updating block {height}: {e}")
            return False
    
    def update_all_cids(self, max_blocks=1000):
        """Update all CIDs in the database"""
        self.log("🔄 Starting comprehensive CID update for all blocks...")
        ...
        for height in range(start_height, latest_height + 1):
            try:
                ...
                if self.is_old_cid_format(current_cid):
                    ...
                    if new_cid:
                        # Update block with new CID
                        if self.update_block_cid_via_api(height, new_cid):
                            updated_count += 1
                            self.log(f"✅ Updated block {height}: {current_cid} → {new_cid}")
                        else:
                            error_count += 1
                            self.log(f"⚠️  Failed to update block {height}")
                    else:
                        error_count += 1
                        self.log(f"⚠️  Could not generate CID for block {height}")
                else:
                    skipped_count += 1
                
                # Progress indicator
                if height % 100 == 0:
                    self.log(f"📊 Processed {height - start_height + 1} blocks... (Updated: {updated_count}, Skipped: {skipped_count}, Errors: {error_count})")
    ...
    def verify_cids(self, sample_size=10):
        ...
        try:
            ...
            self.log(f"❌ CID verification failed: {e}")
            return False

def main():
    """Main function"""
    print("🔄 COINjecture Database CID Updater")
    print("=" * 50)
    
    # Create updater
    updater = DatabaseCIDUpdater()
    
    # Update all CIDs
    success = updater.update_all_cids(max_blocks=2000)  # Update more blocks
    
    if success:
        # Verify the update
        if updater.verify_cids():
            print("\n✅ CID update completed successfully!")
            print("🎯 All blocks now have valid base58btc CIDs")
