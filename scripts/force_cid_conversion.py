#!/usr/bin/env python3
"""
Force CID Conversion
Aggressively converts all CIDs from regular base58 to base58btc
"""
...
class ForceCIDConverter:
    def __init__(self):
        self.converted_cids = []
        self.errors = []
        
    def force_convert_cid(self, cid: str) -> Optional[str]:
        """Force convert any CID to base58btc format"""
        try:
            if not cid or len(cid) != 46:
                return None
            
            # Try to decode as regular base58 first
            try:
                raw_bytes = base58.b58decode(cid)
            except Exception:
                # If that fails, try base58btc
                try:
                    raw_bytes = base58.b58decode(cid, alphabet=base58.ALPHABET_BTC)
                    # If this works, it's already base58btc
                    return cid
                except Exception:
                    return None
            ...
            for row in rows:
                if 'cid' in row and row['cid']:
                    ...
                    if self.has_invalid_characters(old_cid):
                        new_cid = self.force_convert_cid(old_cid)
                        if new_cid:
                            row['cid'] = new_cid
                            cid_updates += 1
            ...
            for key, value in data.items():
                if key == 'cid' and isinstance(value, str):
                    if self.has_invalid_characters(value):
                        new_cid = self.force_convert_cid(value)
                        if new_cid:
                            data[key] = new_cid
                            cid_updates += 1
                            self.converted_cids.append({
    ...
    def fix_all_data_files(self, data_dir: str = "kaggle_data") -> Dict[str, int]:
        ...
        print(f"🚀 Force converting CIDs in {data_dir}")
    ...
    def save_conversion_log(self, results: Dict[str, int]) -> str:
        ...
        log_file = f"force_cid_conversion_log_{int(time.time())}.json"
    ...
    def run_conversion(self, data_dir: str = "kaggle_data") -> bool:
        ...
        print("🎯 COINjecture Force CID Conversion")
        print("=" * 40)
...
def main():
    ...
    converter = ForceCIDConverter()
    success = converter.run_conversion(data_dir)
    
    if success:
        print("\n✅ Force CID conversion completed successfully!")
    else:
        print("\n❌ Force CID conversion completed with errors.")
